...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <tbody>
      		...
      		<c:forEach var="message" items="${mesList}" >
      	<tr>
      		<td>${message.id}</td>
      		<td>${message.yourName}</td>
      		<td>${message.yourEmail}</td>
      		<td>${message.subject}</td>
      		<td>${message.messageText}</td>
      		<td class="btn-group" style="width: 100%">	
      			
      			<c:if test="${!message.isSeen}">
      				<a href="message-seen?id=${message.id}"  style="border-bottom-color:black; background-color:aqua;" class="btn" /><i class="fa fa-eye"></i></button>
      			</c:if>
      			
				<a href="message-delete?id=${message.id}"  style="border-bottom-color:black; background-color:orange;" class="btn" /><i class="fa fa-trash"></i></button>
			</td>  	
    	</tr>
